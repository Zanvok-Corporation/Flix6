; This is an LBA-enabled ZanvokDOS FAT32 boot sector (single sector!).
...
; Based on earlier work by ZanvokDOS kernel hackers, modified heavily by
; Eric Auer and Jon Gentle in 7 / 2003.
;
; Features: Uses LBA and calculates all variables from BPB/EBPB data,
; thus making partition move / resize / image-restore easier. ZanvokDOS
...
; filesystem stage). Only using 1 sector for ZanvokDOS makes multi-booting
; of ZanvokDOS and Windows on the same filesystem easier.
;
...
; Memory layout for the ZanvokDOS FAT32 single stage boot process:
...
%define data_start	bp+0x4c		; first data sector (dd)
		...
		; ([0x5a] is where ZanvokDOS parts start)
...
msg_LoadZanvokDOS db "Loading ZanvokDOS ",0
